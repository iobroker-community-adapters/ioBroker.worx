...
class mower extends EventEmitter {
    constructor(data, worxInstance) {
        ...
        this.worxInstance = worxInstance;
    }
    /**
    ...
    weather() {
        ...
        return new Promise(function (fulfill, reject) {
            that.worxInstance._get2('GET', `product-items/${that.serial}/weather/current`, null, function (err, data) {
                if (err) reject(err);
    ...
    status() {
        const that = this;
        return new Promise(function (fulfill, reject) {
            that.worxInstance._get2('GET', `product-items/${that.serial}/status`, null, function (err, data) {
                if (err) reject(err);
                that.message = data;
                fulfill(data);
            });
                            ...
                            that.getUserDevices()
                                .then((data) => {
                                    ...
                                    data.forEach(function (element, index) {
                                        const mow = new mower(element, that);
                                        that.mower.push(mow);
                                        that.emit('found', mow);
                                    });
                                    ...
                                    if (that.UserCert.active === true) {
                                        ...
                                        that.adapter.log.warn('maybe your connection is blocked from Worx, please test start button, if not working, try again in 24h');
                                        that.adapter.log.warn('DON`T CONTACT THE OFFICIAL WORX SUPPORT BECAUSE THIS IS AN INOFFICAL APP !!!!!!!!!!!');
                                        that._connectMqtt();
                                    }
