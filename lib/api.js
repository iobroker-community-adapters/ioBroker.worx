...
class Worx extends EventEmitter {
            ...
            .then((data) => {
                ...
                that.getUserCert()
                    .then((data) => {
                        ...
                        that.getUserDevices()
                            .then((data) => {
                                that.adapter.log.info(data.length + ' Recieve User Devices');
                                that.adapter.log.debug(+JSON.stringify(data));
                                that.UserDevices = data;

                                data.forEach(function (element, index) {
                                    const mow = new mower(element);
                                    that.mower.push(mow);
                                    that.emit('found', mow, Object.keys(data).length);
                                });
    ...
    async start_mqtt(data_worx) {
        const that = this;

        if (data_worx && data_worx.uuid && data_worx.uuid !== '') {
            UUID = data_worx.uuid;
            that.adapter.log.info('Use UUID from Worx Cloud!');
        }
    ...
    async _getTocken() {
        ...
        const codeResponse = await requestClient({
            ...
            .then((response) => {
                ...
                errorText && this.adapter.log.error(errorText[1]);
                return;
            })
            .catch((error) => {
