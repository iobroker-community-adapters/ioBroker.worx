    ...
    async cleanupRaw(raw) {
        try {
            ...
            typeof mower.auto_schedule_settings.exclusion_scheduler.days === "object"
                ...
                Object.keys(mower.auto_schedule_settings.exclusion_scheduler.days).forEach(async (key) => {
                    if (Object.keys(mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"]).length < 4) {
                        generic = 0;
                        irrigation = 0;
                        ...
                        if (Object.keys(slots_save).length === 1) {
                            ...
                            mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].start_time = slots_save[0].start_time;
                            mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].duration = slots_save[0].duration;
                            mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].reason = slots_save[0].reason;
                            ...
                            Object.keys(slots_save).forEach(async (sl) => {
                                ...
                                mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].start_time = slots_save[sl].start_time;
                                mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].duration = slots_save[sl].duration;
                                mower.auto_schedule_settings.exclusion_scheduler.days[key]["slots"][set_arr].reason = slots_save[sl].reason;
                            });
                        }
                    }
                });
            }
            if (mower && mower.auto_schedule_settings && mower.auto_schedule_settings.nutrition !== "undefined") {
                if (mower.auto_schedule_settings.nutrition === null) mower.auto_schedule_settings.nutrition = {"n": 0, "p": 0, "k": 0};
            }
            if (mower && mower.pin_code !== "undefined") {
                if (mower.pin_code === null) mower.pin_code = 0;
            }
            if (mower && mower.sim !== "undefined") {
                if (mower.sim === null) delete mower.sim;
            }
            if (mower && mower.app_settings !== "undefined") {
                if (mower.app_settings === null) delete mower.app_settings;
            }
            if (mower && mower.pending_radio_link_validation !== "undefined") {
                if (mower.pending_radio_link_validation === null) delete mower.pending_radio_link_validation;
            }
            if (mower && mower.last_gps_status !== "undefined") {
                if (mower.last_gps_status === null) delete mower.last_gps_status;
            }
            if (mower && mower.blade_work_time_reset_at !== "undefined") {
                if (mower.blade_work_time_reset_at === null) mower.blade_work_time_reset_at = "";
            }
            if (mower && mower.battery_charge_cycles_reset_at !== "undefined") {
