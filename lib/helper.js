...
module.exports = {
    ...
    async createDevices(mower, dev) {
        ...
        if (vision != "NO") {
            if (
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    if (day_slots.length > higher_slot) higher_slot = day_slots.length;
                }
        ...
        if (
            ...
            await this.setStateAsync(`${mower.serial_number}.mower.waitRainCountdown`, {
                val: 0,
                ack: true,
            });
            common = {
    ...
    async setStates(mower) {
        ...
        try {
            ...
            /* Countndown disabled because Vision is not yet supported
            if (data.dat && data.dat.rain && data.dat.rain.s != null) {
                await this.setStateAsync(`${mower.serial_number}.mower.waitRainSensor`, {
                    val: data.dat.rain.s,
                    ack: true,
                });
            ...
            */
            ...
            if (vision != "NO") {
                if (data.dat && data.dat.fw != null) {
                    await this.setStateAsync(`${mower.serial_number}.mower.firmware_body`, {
                        val: data.dat.fw.toString(),
                        ack: true,
                    });
                }
                if (data.dat && data.dat.head != null && data.dat.head.fw != null) {
                    await this.setStateAsync(`${mower.serial_number}.mower.firmware_head`, {
                ...
                if (data.cfg && data.cfg.rtk && data.cfg.rtk.map != null) {
                    await this.setStateAsync(`${mower.serial_number}.rtk_zones.map_id`, {
                        val: data.cfg.rtk.map,
                        ack: true,
                    });
                }
                if (data.cfg && data.cfg.rtk && data.cfg.rtk.ck != null) {
                    await this.setStateAsync(`${mower.serial_number}.rtk_zones.map_ck`, {
                        val: data.cfg.rtk.ck,
                        ack: true,
                    });
                }
                if (data.cfg && data.cfg.rtk && data.cfg.rtk.st != null) {
                    await this.setStateAsync(`${mower.serial_number}.rtk_zones.map_status`, {
                        val: data.cfg.rtk.st,
                        ack: true,
                    });
                }
                if (data.dat && data.dat.rfid) {
                    await this.setStateAsync(`${mower.serial_number}.mower.rfidStatus`, {
                        val: data.dat.rfid.status,
                        ack: true,
                    });
                }
                if (data.dat && data.dat.cam) {
                    await this.setStateAsync(`${mower.serial_number}.mower.cameraStatus`, {
                        val: data.dat.cam.status,
                        ack: true,
                    });
                    await this.setStateAsync(`${mower.serial_number}.mower.cameraError`, {
                ...
                if (data.cfg.sc && data.cfg.sc.once && data.cfg.sc.once.time != null) {
                    ...
                    await this.setStateAsync(`${mower.serial_number}.mower.oneTimeZones`, {
                ...
                if (data.cfg.sc && data.cfg.sc.ots) {
                    ...
                    await this.setStateAsync(`${mower.serial_number}.mower.oneTimeWorkTime`, {
                        val: data.cfg.sc.ots.wtm ? 0 : data.cfg.sc.ots.wtm,
                        ack: true,
                    });
                    await this.setStateAsync(`${mower.serial_number}.mower.oneTimeJson`, {
            ...
            // edgecutting
            if (this.modules[mower.serial_number] != null) {
            ...
            if (data.cfg.modules && data.cfg.modules["HL"]) {
                ...
                await this.setStateAsync(mower.serial_number + ".modules.HL.enabled", {
            ...
            if (data.cfg.modules && data.cfg.modules.EA) {
                ...
                await this.setStateAsync(`${mower.serial_number}.modules.EA.height`, {
                    val:
