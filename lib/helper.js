...
exports.AmazonRootCA1 = `-----BEGIN CERTIFICATE-----
...
module.exports = {
    ...
    async checkObjects() {
        ...
        this.stateCheck.push(`${this.namespace}.info`);
        this.stateCheck.push(`${this.namespace}.info.connection`);
        this.stateCheck.push(`${this.namespace}.info.mowerStatus`);
        this.stateCheck.push(`${this.namespace}.info.mowerError`);
        this.stateCheck.push(`${this.namespace}.session`);
        this.stateCheck.push(`${this.namespace}.rateLimit`);
        this.stateCheck.push(`${this.namespace}.loginInfo`);
        this.stateCheck.push(`${this.namespace}.blocking`);
        await this.sleep(10 * 1000);
        const allId = await this.getAdapterObjectsAsync();
        let found = 0;
        let del = 0;
        for (const id in allId) {
            if (!this.stateCheck.includes(id) && !id.includes(`rawMqtt`) && !id.includes(`.product`)) {
                ++del;
        ...
        this.log.debug(`Check ${found + del} objects and delete ${del} object(s)!`);
        this.stateCheck = [];
    },
    ...
    async createDevices(mower, dev, error_states) {
        ...
        const native_notify = {
            notify:
                nativ_check && nativ_check.native && nativ_check.native.notify != null
                    ...
                    : true,
            notify_excluded:
                nativ_check && nativ_check.native && nativ_check.native.notify_excluded != null
                    ? nativ_check.native.notify_excluded
                    : "4,5,16,18",
        };
        this.modules[mower.serial_number]["notify"] = this.config.notification ? native_notify.notify : false;
        this.modules[mower.serial_number]["notify_excluded"] = native_notify.notify_excluded
            .replace(/ /g, "")
            .split(",");
        common = {
            name: mower.name,
        ...
        if (this.deviceType[mower.serial_number] != "wire") {
            if (
                ...
                if (this.deviceType[mower.serial_number] === "vision") {
                    ...
                    this.log.info(`Found Vision Cloud - Create/Update calendar for ${mower.serial_number}!`);
                } else {
            ...
            if (this.deviceType[mower.serial_number] === "cloud" || this.deviceType[mower.serial_number] === "rtk") {
                for (const o of objects.oneTimeSchedule) {
                    if (this.deviceProtocol[mower.serial_number] == 1 && o.common.write != null) {
                        o.common.write = false;
                    }
                ...
                await this.createDataPoint(`${mower.serial_number}.mower.latitude`, common, "state");
                common = {
                    name: {
                        en: "Longitude",
                        de: "Längengrad",
                ...
                await this.createDataPoint(`${mower.serial_number}.mower.longitude`, common, "state");
            }
            if (this.deviceType[mower.serial_number] === "cloud" || this.deviceType[mower.serial_number] === "vision") {
                states_cmd = {
                    1: "Start",
                    2: "Stop",
                    3: "Home",
                ...
                common = {
                    ...
                    role: "switch",
                    read: true,
                    write: this.deviceProtocol[mower.serial_number] == 1 ? false : true,
                    def: false,
                };
                await this.createDataPoint(`${mower.serial_number}.mower.cutOverSlabs`, common, "state");
            }
            if (this.deviceType[mower.serial_number] === "rtk" || this.deviceType[mower.serial_number] === "cloud") {
            ...
            if (this.deviceType[mower.serial_number] === "rtk") {
                states_cmd = {
                    1: "Start",
                    2: "Stop",
                    3: "Home",
        ...
        if (this.notifyAvailable && this.config.notification) {
            common = {
                name: {
            ...
            common = {
                name: {
                    en: "Exclude error IDs (separate IDs with comma)",
                    de: "Fehler-IDs ausschließen (IDs durch Komma trennen)",
                    ru: "Исключить идентификаторы ошибок (разделите идентификаторы запятыми)",
                    pt: "Excluir IDs de erro (separar IDs com vírgula)",
                    nl: "Fout-ID's uitsluiten (scheid ID's met komma's)",
                    fr: "Exclure les ID d'erreur (séparer les ID par une virgule)",
                    it: "Escludi gli ID di errore (separa gli ID con una virgola)",
                    es: "Excluir identificadores de error (separar los identificadores con una coma)",
                    pl: "Wyklucz identyfikatory błędów (oddziel identyfikatory przecinkami)",
                    uk: "Виключити ідентифікатори помилок (розділяти ідентифікатори комами)",
                    "zh-cn": "排除错误 ID（用逗号分隔 ID）",
                },
                type: "string",
                role: "state",
                read: true,
                write: true,
                desc: "Exclude error IDs (separate IDs with comma)",
                def: "4,5,16,18",
            };
            await this.createDataPoint(`${mower.serial_number}.mower.notification_excluded`, common, "state");
        } else {
            await this.delObjectAsync(`${mower.serial_number}.mower.notification`, {
                recursive: true,
            });
            await this.delObjectAsync(`${mower.serial_number}.mower.notification_excluded`, {
                recursive: true,
            });
        }
        common = {
        ...
        if (this.deviceProtocol[mower.serial_number] != 1) {
            ...
            await this.delObjectAsync(`${mower.serial_number}.mower.edgecut`, { recursive: true });
        }
        common = {
            name: {
                en: "Start or Stop",
        ...
        if (this.modules[mower.serial_number] != null && this.deviceType[mower.serial_number] === "wire") {
            if (
                mower.last_status &&
                mower.last_status.payload &&
    ...
    async setStates(mower) {
        ...
        try {
            ...
            if (this.deviceType[mower.serial_number] === "cloud" || this.deviceType[mower.serial_number] === "rtk") {
                if (data.dat && data.dat.rtk && data.dat.rtk.pos && data.dat.rtk.pos[0] != null) {
                    await this.setState(`${mower.serial_number}.mower.latitude`, {
                        val: data.dat.rtk.pos[0],
                        ack: true,
                    });
                }
                if (data.dat && data.dat.rtk && data.dat.rtk.pos && data.dat.rtk.pos[1] != null) {
                    await this.setState(`${mower.serial_number}.mower.longitude`, {
                        val: data.dat.rtk.pos[1],
                        ack: true,
                    });
                }
                if (data.cfg && data.cfg.cut != null) {
                    await this.setState(`${mower.serial_number}.mower.oneTimeJsonRTK`, {
                        val: JSON.stringify(data.cfg.cut),
                        ack: true,
                    });
                }
            }
            ...
            if (this.deviceType[mower.serial_number] != "wire") {
                ...
                if (data.cfg.sc && data.cfg.sc.slots) {
                    data.cfg.sc.slots = await this.evaluateVisionCalendar(
                        mower,
                        data.cfg.sc.slots,
                        this.deviceType[mower.serial_number],
                    );
                }
                if (data.cfg.rtk && data.cfg.rtk.zs) {
                    await this.evaluateRTKZone(mower, false);
                }
                if (this.deviceType[mower.serial_number] === "rtk") {
            ...
            //NL Module
            if (data.cfg.modules["NL"] && data.cfg.modules["NL"].conn && data.cfg.modules["NL"].stat) {
                let conn_count = data.cfg.modules["NL"].conn.length;
                if (!this.modules[mower.serial_number]["NL"]) {
                    for (const o of objects.module_NL) {
                        await this.createDataPoint(
                        ...
                        this.log.info(`NearLink Module found! Create States : ${o._id}`);
                    }
                    if (conn_count > 0) {
                        for (let i = 0; i < conn_count; i++) {
                            for (const o of objects.module_NL_CONN) {
                                const conn_id = o._id.replace("_X", `_${i}`);
                                await this.createDataPoint(
                                    `${mower.serial_number}.modules.${conn_id}`,
                                    o.common,
                                    o.type,
                                    o.native,
                                );
                                this.log.info(`NearLink Connection found! Create States : ${conn_id}`);
                            }
                        }
                    }
                }
                this.modules[mower.serial_number]["NL"] = data.cfg.modules["NL"];
                await this.setState(`${mower.serial_number}.modules.NL.error`, {
                    val: data.cfg.modules["NL"]["error"],
                    ack: true,
                });
                await this.setState(`${mower.serial_number}.modules.NL.status`, {
                    val: data.cfg.modules["NL"]["stat"],
                    ack: true,
                });
                if (conn_count > 0) {
                    for (let i = 0; i < conn_count; i++) {
                        if (data.cfg.modules["NL"].conn[i].mac != null) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.mac`, {
                                val: data.cfg.modules["NL"].conn[i].mac,
                                ack: true,
                            });
                        }
                        if (data.cfg.modules["NL"].conn[i].rssi != null) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.rssi`, {
                                val: data.cfg.modules["NL"].conn[i].rssi,
                                ack: true,
                            });
                        }
                        if (data.cfg.modules["NL"].conn[i].type != null) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.type`, {
                                val: data.cfg.modules["NL"].conn[i].type,
                                ack: true,
                            });
                        }
                        if (data.cfg.modules["NL"].conn[i].vers != null) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.vers`, {
                                val: data.cfg.modules["NL"].conn[i].vers,
                                ack: true,
                            });
                        }
                        if (
                            data.cfg.modules["NL"].conn[i].wifi != null &&
                            data.cfg.modules["NL"].conn[i].wifi.rssi != null
                        ) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.wifi_rssi`, {
                                val: data.cfg.modules["NL"].conn[i].wifi.rssi,
                                ack: true,
                            });
                        }
                        if (
                            data.cfg.modules["NL"].conn[i].wifi != null &&
                            data.cfg.modules["NL"].conn[i].wifi.stat != null
                        ) {
                            await this.setState(`${mower.serial_number}.modules.NL.connection_${i}.wifi_stat`, {
                                val: data.cfg.modules["NL"].conn[i].wifi.stat,
                                ack: true,
                            });
                        }
                    }
            ...
            // NL Module
            if (data.cfg.modules && data.cfg.modules.NL) {
                // Read Only
                //NL: {
                //    stat: "ok",
                //    error: 0,
