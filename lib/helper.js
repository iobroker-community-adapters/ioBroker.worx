...
module.exports = {
    async createDevices(mower) {
        ...
        if (mower.capabilities != null && mower.capabilities.includes("vision")) {
            ...
            await this.setObjectNotExistsAsync(`${mower.serial_number}.areas.rfid`, {
                type: "state",
                common: {
                    name: `RFID`,
                    type: "number",
            ...
            await this.setObjectNotExistsAsync(`${mower.serial_number}.mower.log_improvement`, {
                type: "state",
                common: {
                    name: "Information for product improvement",
                    type: "boolean",
                    role: "switch",
                    read: true,
                    write: true,
            ...
            await this.setObjectNotExistsAsync(`${mower.serial_number}.mower.log_troubleshooting`, {
                type: "state",
                common: {
                    name: "Information for troubleshooting",
                    type: "boolean",
                    role: "switch",
                    read: true,
                    write: true,
        ...
        for (const day of this.week) {
            await this.setObjectNotExistsAsync(`${mower.serial_number}.calendar.${day}`, {
                type: "channel",
                common: {
                    name: `${day}`,
                },
        ...
        if (
            ...
            await this.setObjectNotExistsAsync(`${mower.serial_number}.areas.zoneKeeper`, {
                type: "state",
                common: {
                    name: "Multi-ZoneKeeper",
                    type: "boolean",
                    role: "switch",
                    read: true,
                    write: true,
        ...
        try {
            ...
            if (mower.capabilities != null && mower.capabilities.includes("vision")) {
                ...
                await this.setStateAsync(`${mower.serial_number}.mower.mowerActive`, {
                    val: data.cfg.sc && data.cfg.sc.enabled ? true : false,
                    ack: true,
                });
            } else {
                await this.setStateAsync(`${mower.serial_number}.mower.mowerActive`, {
            ...
            //modules
            this.log.info("data.cfg.modules: " + data.cfg.modules);
            this.log.info("this.modules.channel: " + this.modules[mower.serial_number].channel);
    ...
    async createDataPoint(ident, common, types, native) {
        try {
            ...
            if (!obj) {
                ...
                if (ischange) {
                    this.log.warn(`Change common: ${this.namespace}.${ident}`);
                    delete obj["common"];
                    obj["common"] = common;
                    obj["type"] = types;
