...
module.exports = class Json2iob {
    ...
    async parse(path, element, options) {
        try {
            ...
            if (typeof element === "string" || typeof element === "number") {
                let name = element;
                if (typeof element === "number" || path.match("payload.cfg.sc.d")) {
    ...
    async extractArray(element, key, path, options) {
        try {
            ...
            if (Array.isArray(element)) {
                if (!this.alreadyCreatedObjects[path + "." + key] && key !== "") {
                    this.adapter.log.info("path " + path + " key " + key + " element " + element);
                    await this.adapter
                        .setObjectNotExistsAsync(path + "." + key, {
                            type: "channel",
                            common: {
            ...
            for (let index in element) {
                ...
                if (typeof arrayElement === "string" && path.match("payload.cfg.sc.d")) {
                    await this.parse(path + "." + index, arrayElement, options);
                    continue;
                }
